...
let findCountryIndex=(data, neighbor)=>{
   let index=neighbor.findIndex(e=>e.includes(neighbor));
   return index;
}
let country=(data,Gdata)=>{
   ...
   <div class="x2-column">
       ...
       <h5> Languages : ${data.languages ?
           data.languages.map(function(item){ return `<a href="country.html?id=${findCountryIndex(Gdata, neighbor)}">`+item.name+"</a>"}).join(" ")
...
async function Data_app() {
    ...
    if(country_id){
        main.innerHTML =country(data[country_id],data.name);
        country_info= data.map((item, index)=>{
            return{name:item.name}
        })
        console.log(typeof(country_info[1].name));
    }else{
        country_info = data.map((item, index)=>{
